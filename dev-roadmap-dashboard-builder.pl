...
my ( $opt, $usage ) = describe_options(
    ...
    [
        'slack|s=s',
        "Slack webhook URL",
        { required => 0, default => $ENV{SLACK_URL} }
    ],
    [],
    [
        'quay-username=s',
...
foreach my $q (@queues) {
    ...
    foreach my $ticket_id (@ids) {
        ...
        my ( $bug_id, $others ) = $ticket->{'CF.{Community Bug}'} ? split( ',', $ticket->{'CF.{Community Bug}'} ) : (undef,undef);
        ...
        DumpFile("_source/$bug_id.md", $yaml );
        qx{ echo '---' >> _source/$bug_id.md };
    }
}
